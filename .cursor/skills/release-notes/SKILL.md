...
## 절차
1. 커밋 목록을 수집한다.
   - **Windows (PowerShell):** `.\.cursor\skills\release-notes\scripts\collect_commits.ps1 -From <이전태그> -To <현재태그>`
   - **macOS / Linux / Git Bash:** `bash .cursor/skills/release-notes/scripts/collect_commits.sh <이전태그> <현재태그>`
   - Windows CMD에는 `chmod`가 없으므로 `chmod +x`는 필요하지 않다.
